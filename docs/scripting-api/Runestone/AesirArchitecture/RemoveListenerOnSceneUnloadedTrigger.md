...
- 任意场景卸载时自动移除该场景注册的监听。按场景名分桶，场景 A 卸载不会误杀场景 B 的监听。 挂载在 [Aesir Architecture] GameObject 上，通过 Instance 访问。
...
| 普通方法名称 | 注释 |
| :--- | :--- | 
| `public void AddRemoveListenerHandle` | 添加监听句柄，使其在注册时所属场景卸载时自动移除 |
...
| 属性签名 | 注释 |
| :--- | :--- |
| `public static RemoveListenerOnSceneUnloadedTrigger Instance { get; }` | 获取全局唯一的场景卸载监听移除器实例 |
