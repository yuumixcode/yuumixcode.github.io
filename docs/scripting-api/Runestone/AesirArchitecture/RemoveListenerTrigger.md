...
``` csharp
public abstract class RemoveListenerTrigger : Runestone.AesirArchitecture.AesirMonoBehaviour, 
...
| 普通方法名称 | 注释 |
| :--- | :--- | 
| `public void AddRemoveListenerHandle` | 添加监听句柄，使其在调用条件满足时自动移除 |
| `protected void RemoveAllListeners` | 移除所有已注册的监听并清空列表 |
